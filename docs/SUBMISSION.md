...
Live StudioNet evidence now verifies `OBSERVED` (case 2), `INCONCLUSIVE` (case 3), retrospective `NOT_OBSERVED` with `can_rely_on_absence() == false` (case 4), and fail-closed `EXTERNAL_FAILURE`/retry mechanics (cases 1 and 5). Case 5 proves the retry delay and a second state-changing attempt with the same definition hash, although its SEC source remained unavailable and recovery has not finalized. The one core lifecycle proof still outstanding is a `PRECOMMITTED` `NOT_OBSERVED` case with `can_rely_on_absence() == true`. Exact transactions, classifications and hashes are recorded in `docs/LIVE_EVIDENCE.md` and `docs/studionet-lifecycle.json`. The latest GitHub Actions quality workflow is green: 25 Direct Mode contract tests + 9 static/source tests pass, GenVM lint/schema pass, and frontend typecheck/build pass. Vercel reports a successful deployment status; the public production URL/browser verification still needs to be recorded.